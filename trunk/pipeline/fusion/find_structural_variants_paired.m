...
function variants = find_structural_variants_paired(reads, varargin)
...
if any(rx(reads.paired, 'single'))
	error('Paired end reads are required.');
end

for s = 1:S
	fprintf(1, 'Analyzing sample %s...\n', reads.meta.sample_id{s});
	candidates{s} = find_anchor_based_variants(filter(reads, s), min_distance);
end
...
for s = 1:S
	al = all_alignments(filter(alignments, s));
...
function candidates = find_anchor_based_variants(reads, min_distance)
	
global organism;
chromosomes = organism.Chromosomes;

fprintf(1, 'Aligning paired end reads to the genome...\n');
alignments = bowtie_align(reads, 'genome', '-v0 -m1');
...
fprintf(1, 'Found %d potential structural variants.\n', length(variant_map));
