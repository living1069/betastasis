function FeatureMatrix(div) {
	...
	// represent rows.
	this.setData = function(data, cols) {
		this.cols = cols;
		...
		if (vis == undefined) {
			...
			this.legend.anchor('right').add(pv.Label)
	...
	this.updateData = function() {
		...
		vis.width(sample_labelsize + this.cols.length * w + 20 + legend_width)
			.height(Math.max(feature_labelsize + this.rows.length * h + 30, 400));
		...
		matrix.data(this.rows)
			...
			.height(h)
			.fillStyle(function(t,d) { return self.cellColor(d[this.index], self.cols[this.parent.index]); })
			.strokeStyle("white").lineWidth(1).antialias(false);
		...
		this.drag_bar.data(this.selected)
			...
			.strokeStyle("none").antialias(false);
		...
		this.legend_title.data(['Expression', 'CNA', 'Mutation'])
			.font('14px sans-serif')
			.left(sample_labelsize + this.cols.length * w + 22)
			.top(function() {
				var pos = [0, 84, 168];
				return feature_labelsize + 14 + pos[this.index];
			});

		this.legend.data(treshhold['legend'])
			.left(sample_labelsize + this.cols.length * w + 30)
			.top(function() { return feature_labelsize + 22 + 12 * this.index; })
			.fillStyle(function(d) {
				var cell_vals = [ 2, 1, 0, -1, -2, -100, -100, 2, 1, 0, -1, -2, -100, -100, 0,10,20,30,40 ];
