// Authors: Kalle Leinonen <kalle.leinonen@tut.fi>
//          Matti Annala <matti.annala@tut.fi>
...
function ExprBoxplot(div) {
    ...
    this.set_data = function(subtype_expr) {
		...
		if (this.diag_labels) {
			this.type_labels.textAngle(Math.PI / 4).bottom(-20);
		} else {
			this.type_labels.textAngle(0).bottom(-25);
			if (subtype_expr.length > 20) {
				this.type_labels.font("8pt Arial").textAlign('center');
			}
		}
		...
		this.expr_boxes.minmax
			...
			.visible(function(d) { return isFinite(d); });
