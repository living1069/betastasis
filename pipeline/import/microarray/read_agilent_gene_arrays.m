...
for s = 1:S
	...
	if s == 1
		gene_expr.rows.gene = data{rx(headers, 'SystematicName')};
		G = length(gene_expr.rows.gene);
		gene_expr.mean(G, S*2) = 0;   % Preallocate space
	else
		if ~strcmp(data{rx(headers, 'SystematicName')}, gene_expr.rows.gene)
...
gene_expr = filter_rows(gene_expr, ...
	~rx(gene_expr.rows.gene, 'Corner|Control'));
gene_expr = filter_rows(gene_expr, ...
	rx(gene_expr.rows.gene, '^(NM_|XM_|NR_|XR_|ENST)'));
