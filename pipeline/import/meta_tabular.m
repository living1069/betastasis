...
%    
%    META = META_TABULAR(META, TLM) reads patient and sample specific metadata
...
function metadata = meta_tabular(metadata, filepath, varargin)
...
if regexpi(filepath, '.+\.txt')
	fprintf(1, 'Reading tabular metadata in tab delimited format...\n');
	fid = fopen(filepath);
...
elseif regexpi(filepath, '.+\.xls')
	
	fprintf(1, 'Reading tabular metadata in XLS spreadsheet format...\n');
	warning off;
...
elseif regexpi(filepath, '.+\.xlsx')
	error 'XLSX spreadsheets not supported. Please convert to XLS.';
end
...
for k = 1:length(column_names)
	...
	if regexpi(column_names{k}, '^(Patient.?)?Age$')
		metadata.Patient.Age = nan(N, 1);
		metadata.Patient.Age(found) = str2double(data{k});
		continue;
	end
	...
	if regexpi(column_names{k}, '(Patient.?)?Gender|Sex')
		gender = data{k};
	...
	if regexpi(column_names{k}, 'Height')
		metadata.Patient.Height = nan(N, 1);
		metadata.Patient.Height(found) = str2double(data{k});
		continue;
	end

	if regexpi(column_names{k}, 'Weight')
		metadata.Patient.Weight = nan(N, 1);
		metadata.Patient.Weight(found) = str2double(data{k});
		continue;
	end
	...
	field = genvarname(column_names{k});
