...
if rx(variant_file, '^.*\.vcf$')
	out_prefix = strrep(variant_file, '.vcf', '');
	...
	convert_cmd = sprintf(['convert2annovar.pl --format vcf4 --includeinfo ' ...
		'--allallele --snpqual %d %s | sed "s/\t\t/\t/"'], ...
		variant_phred_threshold, variant_file);
...
humandb_dir = '/home/csbgroup/tools/annovar*/humandb';
...
% Determine which variants are found in DARNED.
unix(sprintf([ ...
	'annotate_variation.pl --buildver hg19 --filter --outfile %s ' ...
	'--dbtype generic --genericdbfile darned_31-12-2011_hg19.annovar.txt ' ...
	'<(%s) %s'], [out_prefix '.darned'], convert_cmd, humandb_dir));
...
[data, headers] = readtable('variants.exonic_variant_function', ...
	'HeaderLines', 0);
exonic_func_map = containers.Map( ...
	strcat(data{4},':',data{5},'-',data{6},':',data{7},'>',data{8}), ...
	strcat(data{2}, {sprintf('\t')}, data{3}));

[data, headers] = readtable('variants.hg19_snp132_dropped', 'HeaderLines', 0);
dbsnp_map = containers.Map( ...
	strcat(data{3},':',data{4},'-',data{5},':',data{6},'>',data{7}), ...
	data{2});

[data, headers] = readtable('variants.hg19_ALL.sites.2011_05_dropped', ...
	'HeaderLines', 0);
kgenomes_map = containers.Map( ...
	strcat(data{3},':',data{4},'-',data{5},':',data{6},'>',data{7}), ...
	data{2});
	
%[data, headers] = readtable('variants.darned.hg19_generic_dropped', ...
%	'HeaderLines', 0);
%darned_map = containers.Map( ...
%	strcat(data{3},':',data{4},'-',data{5},':',data{6},'>',data{7}), ...
%	repmat({'YES'}, 1, length(data{3})));
	
[data, headers] = readtable('variants.cosmic.hg19_bed', 'HeaderLines', 0);
...
[data, headers] = readtable('variants.variant_function');
...
fprintf(1, '%d / %d (%.1f%%) variants found in dbSNP v132.\n', ...
...
fprintf(1, '%d / %d (%.1f%%) variants found in 1000 Genomes.\n', ...
	sum(valid), length(valid), sum(valid) / length(valid) * 100);
	
%valid = darned_map.isKey(keys);
%darned = repmat({'-'}, length(data{1}), 1);
%darned(valid) = darned_map.values(keys(valid)),
%fprintf(1, '%d / %d (%.1%%) variants found in DARNED.\n', ...
%	sum(valid), length(valid), sum(valid) / length(valid) * 100);
...
fprintf(1, '%d / %d (%.1f%%) variants found in COSMIC.\n', ...
	sum(valid), length(valid), sum(valid) / length(valid) * 100);
...
%		dbsnps{k},  kgenomes{k}, cosmic{k}, darned{k});
