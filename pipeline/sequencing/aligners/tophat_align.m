function alignments = tophat_align(reads, index, flags, varargin)

global organism;
global pipeline_config;

tmp = temporary('tophat_align');
...
for s = 1:S
	...
	fprintf('-> Invoking Tophat with flags "%s"...\n', flags);
	...
	elseif paired && ignore_pairs == true
		...
		[status, out] = unix(sprintf( ...
			...
			[extracted.url{s} '_1.fa'], [extracted.url{s} '_2.fa'], ...
		...
		[status, out] = unix(sprintf( ...
			['tophat %s -x %s %s | ' ...
			'%s/tools/samtools/samtools view -Sb -o %s -'], ...
			flags, index_name, [extracted.url{s} '.fa'], ppath, output_file));
		if status ~= 0, error('Bowtie2 read alignment failed:\n%s\n', out); end
