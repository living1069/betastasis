	...
	this.setGene = function(data) {
		...
		for (var k = 0; k < this.tx_names.length; k++) {
			...
			for (var e = 0; e < tx.exons.length-1; e++) {
				var exon_start = tx.exons[e];
				var exon_len = tx.exons[e+1] - tx.exons[e];
				this.vis_elems.push({ x: exon_start, y: y_base,
					w: exon_len-1, h: exon_h, color: 0.7,
					tooltip: this.tx_names[k] + ' exon ' + (e+1) + 
						'   \nPosition: ' + exon_start + ' .. ' +
						(exon_start + exon_len - 1)});
			}
			
			this.vis_elems.push({ x: tx.exons[tx.exons.length-1], y: y_base,
				w: tx.length - tx.exons[tx.exons.length-1], h: exon_h,
				color: 0.7,
				tooltip: this.tx_names[k] + ' exon ' + tx.exons.length + 
					'   \nPosition: ' + tx.exons[tx.exons.length-1] + ' .. ' +
					tx.length});
	...
	this.updateData = function() {
		...
		tx_label_links.data(this.tx_names)
			...
			.event('click', function(d) {
				page.location = 'http://www.ncbi.nlm.nih.gov/nuccore?term=' + d;
			});
